...
function [AnsatzObj,EvalIter] = SROptimiseRB(SR,SamplerObj,AnsatzObj)
...
SamplerObj.Operators = {SamplerObj.Operators{1:id-1},...
    OperatorDg(AnsatzObj.Hilbert,HypCub(1,'OBC',0,0),@SR_EnLogDerivCorr),...
    OperatorDg(AnsatzObj.Hilbert,HypCub(1,'OBC',0,0),@SR_LogDerivCorr)}; % Concatenate them into the array.
...
for p=1:SR.Npass
    ...
    % Select random batch of parameters to optimise on this pass.
    [AnsatzObj] = RndBatchSelect(AnsatzObj,SR.OFrac);
    ...
    if numel(ind) > 0
        ...
        dP_N0 = - SR.LRate * minres(AnsatzObj.Hilbert.N * S_N0, F_N0);
    ...
    if p <= SR.PSave
        ...
        if SampFlag == 0 % If errors occur in normal sampling, add extra samples and re-iterate, setting up AnsRef as a backup.
            ...
            elseif abs(dEr) < SR.ESens && encount >= SR.PSave
                ...
                EvalIter = EvalIter(1:p,:); % Truncate EvalIter.
                break
            end
