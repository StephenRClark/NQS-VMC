...
t = -1; U = [0 2 4 6 8 9 10 11 12 14 16 18 20 22 24 28 32 40];
...
SR1 = SetRegularisation(SR1,1e4,1e1,0.95);
SR2 = SetRegularisation(SR2,1e1,1e-2,0.96);

% SR1 = SetLearnRate(SR1,0.1);
...
ModParams.nmag = 0.1; ModParams.nphs = 0; ModParams.A = -0.01; ModParams.B = 0;

ExInds = [1 3 4]; dP = [zeros(4,1); 0.1; zeros(Alpha*N-1,1)];

for d = 1:numel(HilbertNQS)
    
    NQSObj = NQSSHTI(HilbertNQS(d),GraphObj,ModParams,1);
    
    NQSObj.OptInds(ExInds) = 0; NQSObj = PsiUpdate(NQSObj,dP);
    ...
    AnsStr = ['BECR-NQSSHTI-HD' num2str(NQSObj.HDim) ' Sq-Lin Alpha ' num2str(Alpha)];
    ...
    for u = 1:numel(U)
        ...
        save([AnsStr NStr num2str(N) '/BHM 2D U ' num2str(U(u)) NStr num2str(N) ' ' AnsStr ' Logs.mat'],...
...
HamiltonianObj.HParams = [t; U(1)/2];
...
for d = 1:numel(HilbertNQS)
    AnsStr = ['BECR-NQSSHTI-HD' num2str(HilbertNQS(d).d) ' Sq-Lin Alpha ' num2str(Alpha)];
    for u = 1:numel(U)
        load([AnsStr NStr num2str(N) '/BHM 2D U ' num2str(U(u)) NStr num2str(N) ' ' AnsStr ' Logs.mat'],'AnsatzObj');
        ...
        RunTime = toc; EnIter = cell2mat(EnIter);
        save([AnsStr NStr num2str(N) '/BHM 2D U ' num2str(U(u)) NStr num2str(N) ' ' AnsStr ' FT Logs.mat'],...
            'AnsatzObj','EnIter','RunTime');
        % Save AnsatzObj and run details for later analysis.
    end
end