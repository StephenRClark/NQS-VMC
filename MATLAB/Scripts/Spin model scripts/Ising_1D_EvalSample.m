...
SxOp = Operator1S(HilbertObj,HGraphs(2),@Sx_OpMatEls);
...
Operators = {SzSzOp; SxOp}; HParams = [J, J*G(1)];
...
SzProfiles = zeros(numel(G),N); SzSzProfiles = zeros(numel(G),N);
...
for g = 1:numel(G)
    ...
    load(['Ising 1D G ' num2str(U(g)) ' N ' num2str(N) ' ' AnsStr ' Logs.mat']);
    ...
    SzProfiles(g,:) = reshape(EvalAvg{3},1,N); SzSzProfiles(g,:) = reshape(EvalAvg{4},1,N);
    disp(['Sampling for G = ' num2str(G(g)) ' complete.']);
end

figure(1); hold on; plot(U,real(EneGS));

save(['Ising 1D N ' num2str(N) ' G Scan ' AnsStr ' expectation values.mat'],...
