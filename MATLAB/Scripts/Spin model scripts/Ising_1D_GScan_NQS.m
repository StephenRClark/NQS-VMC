...
% transverse field Ising model in 1D using NQS.
...
SR = SetSRTolerances(SR,3+G(1),1e-6);

SR = SetRegularisation(SR,1e5,1e-3,0.9);
...
Alpha = 1;
ModParams.Nh = Alpha * N; ModParams.a = 0.01; ModParams.b = 0.01; ModParams.W = -0.01;
...
for g = 1:numel(G)
    ...
    RunTime = toc;
    save(['Ising 1D G ' num2str(G(g)) ' N ' num2str(N) ' ' AnsStr ' Logs.mat'],...
