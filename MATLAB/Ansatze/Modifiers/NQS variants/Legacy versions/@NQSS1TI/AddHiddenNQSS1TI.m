...
function [NQSObj] = AddHiddenNQSS1TI(NQSObj,Params)
...
bti = NQSObj.bti; wv = NQSObj.wv; Wv = NQSObj.Wv;
...
if AlphaP < 0
    if abs(AlphaP) >= Alpha0
        ...
        btiF = bti(1:AlphaF); wvF = wv(1:AlphaF,:); WvF = Wv(1:AlphaF,:);
    end
else
    AlphaF = Alpha0 + AlphaP;
    btiF = zeros(AlphaP,1); wvF = zeros(AlphaP,Nv); WvF = zeros(AlphaP,Nv);
    for a = 1:AlphaP
    ...
    btiF = [bti; btiF]; wvF = [wv; wvF]; WvF = [Wv; WvF];
end

% Reassign all fields affected by Nh change.
NQSObj.Np = 2 + AlphaF + 2*AlphaF*Nv; NQSObj.Nh = AlphaF*Ntr; NQSObj.Theta = zeros(AlphaF*Ntr,1);
NQSObj.bti = btiF; NQSObj.wv = wvF; NQSObj.Wv = WvF; NQSObj.OptInds = ones(NQSObj.Np,1); NQSObj.Alpha = AlphaF;
