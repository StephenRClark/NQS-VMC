...
function [Ratio,Update] = PsiRatioNQSMHAI(NQSObj,Diff)
...
for i=1:Diff.num
    ...
    ThetaH_shift = ThetaH_shift + dHv(Diff.pos(i))*NQSObj.WH(:,Diff.pos(i)) + ...
        dMv(Diff.pos(i))*NQSObj.XH(:,Diff.pos(i)); % Change in hidden holon effective angles.
    ThetaM_shift = ThetaM_shift + dMv(Diff.pos(i))*NQSObj.WM(:,Diff.pos(i)) + ...
        dHv(Diff.pos(i))*NQSObj.XM(:,Diff.pos(i)); % Change in hidden multiplon effective angles.
end
