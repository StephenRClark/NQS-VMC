...
function dLogp = LogDerivNQSMHV(NQSObj,Cfg)
...
for a=1:Alpha % Derivatives need to be computed by Alpha sector
        ...
        if NQSObj.OptInds(PIndW) == 1
            for b = 1:Ntr
                ...
                if VInd ~= 0
                    ...
                    if sum(NNInds(1,:) == v) == 0 % Not a special free W coupling
                        dLogp(PIndW) = dLogp(PIndW) - (NQSObj.Mv(VInd)*dThetaH(TInd)) ...
    ...
    for nn = 1:size(NNInds,2) % Select the free X couplings.
        v = NNInds(1,nn); PIndX = 2*(1+Alpha) + Alpha*Nv + (a-1)*size(NNInds,2) + nn;
