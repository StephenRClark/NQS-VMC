classdef CPSX < Modifier
    % CPSX - a Modifier subclass that modifies configuration amplitudes
    % using a Restricted Boltzmann Machine architecture of visible neurons
    % and hidden spins, parameterised as a correlator product state. X for
    ...
    % - CPSX.HDim = 3 - hidden unit dimension.
    % - CPSX.VDim = 3 - visible unit dimension.
    ...
    methods
        function obj = CPSX(Hilbert,Graph,Params,VFlag)
        ...
        function [Ratio,Update] = PsiRatio(obj,Diff)
            ...
            for k = 1:Diff.num
                if obj.IndV(obj.VisInds(Diff.pos(k))) ~= 0
                    Ratio = Ratio * exp(-obj.a(Diff.pos(k),obj.IndV(obj.VisInds(Diff.pos(k)))));
                end
                if obj.IndV(VisIndsP(Diff.pos(k))) ~= 0
                    Ratio = Ratio * exp(obj.a(Diff.pos(k),obj.IndV(VisIndsP(Diff.pos(k)))));
                end
            end
            % Perform the trace over the hidden dimension and take the product of trace
            % ratios.
            Ratio = Ratio * prod((1+sum(exp(ThetaP),2))./(1+sum(exp(obj.Theta),2)));
