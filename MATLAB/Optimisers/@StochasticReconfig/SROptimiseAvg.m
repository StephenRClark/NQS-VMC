...
function [AnsatzObj,EnIter,Params] = SROptimiseAvg(SR,SamplerObj,AnsatzObj)
...
% Overwrite the operators with the SR correlations in the evaluation list:
id = 1; % Index for SR correlations in the list.
SamplerObj.Operators = {OperatorDg(AnsatzObj.Hilbert,HypCub(1,0,0,0),@SR_EnLogDerivCorr),...
    OperatorDg(AnsatzObj.Hilbert,HypCub(1,0,0,0),@SR_LogDerivCorr)}; % Concatenate them into the array.

EnIter = zeros(SR.Npass,id); Params = 0*ParamList(AnsatzObj); PassCheck = 0;
...
for p=1:SR.Npass
    ...
    EnIter(p) = EnAvg/AnsatzObj.Hilbert.N;
    ...
    if SampFlag == 0 % Ordinary sampling, no errant results
        Params = Params + ParamList(AnsatzObj); PassCheck = PassCheck+1;
    end
    fprintf('Energy calculated per site from sampling: %6f\n',EnIter(p));
end

Params = Params/PassCheck; Param0 = AnsatzObj.ParamList; dP = Params - Param0;
