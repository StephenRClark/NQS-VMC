...
function ExSiHExSjQ = ExSiHExSjQ_2D_CorrSamp(Cfg,EnLoc,dLogp,MCMC,Ansatz)
        ...
        for dR1 = 1:L1
            for dR2 = 1:L2
                if dR1 == 1 && dR2 == 1
                    ...
                    Amp(dR1+(dR2-1)*L1) = sum(Cfg_mat(:)) / 2;
                else
                    ...
                    for d = 1:numel(SpSmMatEls)
                        ...
                        Rx = 1+mod(DiffSpSm(d).pos(1)-1,L1); Ry = ceil(DiffSpSm(d).pos(1)/L2);
                        Amp(dR1+(dR2-1)*L1) = Amp(dR1+(dR2-1)*L1) - PsiRatio * SpSmMatEls(d)...
                    ...
                    for d = 1:numel(SmSpMatEls)
                        ...
                        Rx = 1+mod(DiffSmSp(d).pos(1)-1,L1); Ry = ceil(DiffSmSp(d).pos(1)/L2);
                        Amp(dR1+(dR2-1)*L1) = Amp(dR1+(dR2-1)*L1) + PsiRatio * SmSpMatEls(d)...
                            * exp(1i*pi*(Qx(q1)*Rx + Qy(q2)*Ry));
                    end
                end
            end
            ...
            for dR1 = 1:L1
                for dR2 = 1:L2
                    if dR1 == 1 && dR2 == 1
                        ...
                        Cfg_mat = - exp(1i*pi * ( (Qx(q1) * ((1:L1).'*ones(1,L2))) + ...
                            (Qy(q2) * (ones(L1,1)*(1:L2))) )) .* Cfg_mat;
                        HAmp(dR1+(dR2-1)*L1) = HAmp(dR1+(dR2-1)*L1) + PsiRatioH * HMatEls(h) * sum(Cfg_mat(:)) / 2;
                    else
                        [DiffSpSm,SpSmMatEls] = SpSm_2D_PBC_GT_CorrMatEls(HParams,CfgP,[dR1-1, dR2-1]);
                        % Apply S+(r-dR)S-(r) operators to the right.
                        for d = 1:numel(SpSmMatEls)
                            PsiRatio = MCMC.PsiRatio(AnsatzP,DiffSpSm(d));
                            Rx = 1+mod(DiffSpSm(d).pos(1)-1,L1); Ry = ceil(DiffSpSm(d).pos(1)/L1);
                            HAmp(dR1+(dR2-1)*L1) = HAmp(dR1+(dR2-1)*L1) - PsiRatioH * HMatEls(h) * PsiRatio...
                                * SpSmMatEls(d) * exp(1i*pi*(Qx(q1)*Rx + Qy(q2)*Ry));
                        end
                        [DiffSmSp,SmSpMatEls] = SmSp_2D_PBC_GT_CorrMatEls(HParams,CfgP,[dR1-1, dR2-1]);
                        % Apply S-(r-dR)S+(r) operators to the right.
                        for d = 1:numel(SmSpMatEls)
                            PsiRatio = MCMC.PsiRatio(AnsatzP,DiffSmSp(d));
                            Rx = 1+mod(DiffSmSp(d).pos(1)-1,L1); Ry = ceil(DiffSmSp(d).pos(1)/L1);
                            HAmp(dR1+(dR2-1)*L1) = HAmp(dR1+(dR2-1)*L1) + PsiRatioH * HMatEls(h) * PsiRatio...
                                * SmSpMatEls(d) * exp(1i*pi*(Qx(q1)*Rx + Qy(q2)*Ry));
                        end
                    end
                end
            end
