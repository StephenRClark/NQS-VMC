...
function [Diff, OpMatEls] = Sx_MS_OpMatEls(HilbertObj,Cfg,GraphObj,ListIndex)
% Given a spin-1/2 configuration Cfg this function computes the list of
% configurations CfgP's and matrix elements <Cfg|Sx{i,...,j}|CfgP>. Only
...
for n=1:Ng
    ...
    Diff(n).val = -2*reshape(Cfg_vec(List(n,:)),1,size(List,2)); % Flip all relevant spins.
    Diff(n).num = size(List,2); % This operator flips each site.
    Diff(n).sign = 1;
    Diff(n).type = 0;
    Diff(n).index = n;
    OpMatEls(n) = 1; % This Operator acts on all configurations.
end
