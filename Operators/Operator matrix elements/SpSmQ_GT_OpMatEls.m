...
function [Diff, OpMatEls] = SpSmQ_GT_OpMatEls(HilbertObj,Cfg,GraphObj)
...
N = Cfg.N; q = 2*pi*((1:N)-1)/N; % Number of sites in the system.
...
for c = 1:CoOrd
    for n = 1:N
        ...
        if m == n
            ...
            Diff(1+(c-1)*N).pos = 1;
            Diff(1+(c-1)*N).val = 0;
            OpMatEls(1+(c-1)*N,:) = (1/2)*sum((Cfg_vec==-1) .* exp(1i*q.'));
            break
        else
            if m ~= 0
                Diff(n+(c-1)*N).num = 2;
                ...
                OpMatEls(n+(c-1)*N,:) = ((-1)^(SLInds(n) - SLInds(m)))*...
                    (1/2) * (Cfg_vec(n) < 0) * (Cfg_vec(m) > 0) * exp(1i*n*q)/sqrt(N);
            end
        end
    end
end
