...
function [Diff, OpMatEls] = SiSj_S1_OpMatEls(HilbertObj,Cfg,GraphObj)
...
% There will be at most 3*Nbond reachable configurations.
...
for c = 1:CoOrd
    for n=1:N
        ...
        if m ~= 0
            ...
            OpMatEls(DInd) = 1 * (Cfg_vec(n)>-1) * (Cfg_vec(m)<1); % Compute matrix element.
        end
        ...
        if m ~= 0
            ...
            OpMatEls(DInd+1) = 1 * (Cfg_vec(m)>-1) * (Cfg_vec(n)<1); % Compute matrix element.
        end
    end
end
...
for c = 1:CoOrd
    for n=1:N
        ...
        if m ~= 0
            OpMatEls(DInd) = OpMatEls(DInd) + Cfg_vec(n)*Cfg_vec(m); % Compute matrix element.
        end
    end
end
