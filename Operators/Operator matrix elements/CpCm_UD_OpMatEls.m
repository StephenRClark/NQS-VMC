...
function [Diff, OpMatEls] = CpCm_UD_OpMatEls(HilbertObj,Cfg,GraphObj)
...
for c = 1:CoOrd
    for n = 1:N
        ...
        if m ~= 0 && m ~= n
            ...
            Diff(DInd).pos(1) = n;
            Diff(DInd).val(1) = -1; % Remove up fermion from site n.
            Diff(DInd).pos(2) = m;
            Diff(DInd).val(2) = 1; % Add up fermion to site m.
        ...
        elseif m == n % No fermions move, becomes total number counter.
            Diff(1).type = 1;
            Diff(1).pos = 1;
            Diff(1).val = 0;
            Diff(1).num = 0;
            OpMatEls(1) = sum(Cfg_vec(:,1));
            
            Diff(2).type = -1;
            Diff(2).pos = 1;
            Diff(2).val = 0;
            Diff(2).num = 0;
            OpMatEls(2) = sum(Cfg_vec(:,2));
            break
        end
    end
end
