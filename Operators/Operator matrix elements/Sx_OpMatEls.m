...
function [Diff, OpMatEls] = Sx_OpMatEls(HilbertObj,Cfg)
% Given a spin-1/2 configuration Cfg this function computes the list of
% configurations CfgP's and matrix elements <Cfg|Sx{i}|CfgP>. Only the
...
for n=1:N
    ...
    Diff(n).num = 1; % Spin operators create only one difference.
    Diff(n).sign = 1;
    Diff(n).type = 0;
end

% Deal with each S+ contribution:
% Compute all configurations obtained from Cfg by a single spin operator
...
for n=1:N
    ...
    OpMatEls(n) = 1/2; % Compute matrix element
end
