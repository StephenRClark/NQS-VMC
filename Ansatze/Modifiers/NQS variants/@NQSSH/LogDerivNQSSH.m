...
function dLogp = LogDerivNQSSH(NQSObj,Cfg)
...
dLogp((1:(Nv*Nh))+2*(Nv+Nh)) = reshape((dT_SHTrace(NQSObj.Theta,NQSObj.B,NQSObj.HDim)*Cfg_vec.'),Nh*Nv,1); % Insert d/dW.
dLogp((1:Nh)+2*Nv) = dT_SHTrace(NQSObj.Theta,NQSObj.B,NQSObj.HDim); % Insert d/db.
dLogp((1:Nh)+2*Nv+Nh) = dB_SHTrace(NQSObj.Theta,NQSObj.B,NQSObj.HDim); % Insert d/dB.
